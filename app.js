...
function reverseString(str) {
  ...
  // convert date to string (take care of < 10 nums, add 0)
  ...
  // create all date formats variables & return them in an array
  ...
  // check palindrome birth date for all formats
  ...
  // to get next date (add + 1, so take care for month and year)
  ...
  // get previous palindrome bday date
  ...
  let bdayInp = document.querySelector("#date");
  let checkBtn = document.querySelector("#btn");
  let outputDiv = document.querySelector(".result");
  let loadingGIF = document.querySelector('#timer-img');
  
  function clickHandler() {
    loadingGIF.classList.add('hidden');
    let bdayStr = bdayInp.value;
    ...
    if(bdayStr !== ''){
      ...
      if(!isItPalindrome) {
        ...
        if(noOfDays1 > noOfDays2) {
          outputDiv.innerHTML = `The nearest Palindrome date is ${prevDateShow.day}-${prevDateShow.month}-${prevDateShow.year}, you missed by ${noOfDays2} days.`
        }
        else {
          outputDiv.innerHTML = `The nearest Palindrome date is ${nextDateShow.day}-${nextDateShow.month}-${nextDateShow.year}, you missed by ${noOfDays1} days.`
        }
      }
      else {
        outputDiv.innerHTML = `Your birthday is a Palindrome!!!`;
      }
    }
  }
...
checkBtn.addEventListener('click', loadingOn);